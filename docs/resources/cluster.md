...
- `k8s_psat` (Block, Optional) Kubernetes Projected Service Account Token (PSAT) configuration for agent attestation. When configured, the SPIRL agent will use PSAT tokens to attest its identity. (see [below for nested schema](#nestedblock--k8s_psat))
